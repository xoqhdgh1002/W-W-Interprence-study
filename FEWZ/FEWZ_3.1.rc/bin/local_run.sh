...
[ `python scripts/get_scalevar.py $INFILE` = "y" ] && SCALEVAR=3 || SCALEVAR=1
...
if [ $SECT_STEP -gt 0 ] ; then
   ...
   do
      ...
      if [ $j -eq 1 ] ; then
         ...
         ../../$EXEC -i ../../p_$INFILE -h ../../pm_$HISTFILE -o p_$OUTFILE -p ../../../$PDFDIR -l .. -s $(($i-1)) > p_screen.out &
      elif [ $j -eq 3 ] ; then
         cd mscale
         echo "Starting sector $i for down scale variation ..."
         ../../$EXEC -i ../../m_$INFILE -h ../../pm_$HISTFILE -o m_$OUTFILE -p ../../../$PDFDIR -l .. -s $(($i-1)) > m_screen.out &
      fi
      PID=$!
      QUEUE="$QUEUE $PID"
      NUM=$(($NUM+1))
